<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<%@ include file="../include/admin_head.jspf"%>
<title>회원 목록</title>
<script type="text/javascript">
$(function(){
	
	optionSelect(1);
	optionSelect(2);
	optionSelect(3);
	
	$("#btnDelete").click(function(){
		if(confirm('탈퇴처리하시겠습니까?')){
		document.form1.action="${path}/admin/member_delete.do";
		document.form1.submit();
		}
	});
	$("#btnSearch").click(function(){
		document.form1.action="${path}/admin/member_list.do";
		document.form1.submit();
	});
});
function optionSelect(num){
	var selected="";
	var option=''.concat('#searchOption',num);
		if(num==1){
			selected='${searchOp.searchOption1}';
		}else if(num==2) {
			selected='${searchOp.searchOption2}';
		}else {
			selected='${searchOp.orderOption}';
			option='#orderOption';
		}
		console.log(option);
	
	for(i=1; i<=4; i++)	{
		var val = $(option+' option:eq('+i+')').val(); 
		if(selected==val) 
			$(option+' option:eq('+i+')').prop("selected", "selected");
	}
}
</script>
</head>
<body>
	<div class="container">
		<h3>회원관리</h3>
		<form name="form1" class="form-inline" method="post">
		<div class="form-group">
		<table class="table table-striped">
		 <tr>
		  <th>전체회원수</th>
		  <th>오늘 가입한 회원수</th>
		  <th>탈퇴 회원수</th>
		 </tr>
		 <tr>
		  <td>${mcount.member_all}명</td>
		  <td>${mcount.member_today}명</td>
		  <td>${mcount.member_quit}명</td>
		 </tr>
		</table>
		 <table class="table table-bordered">
		 <tr><th>검색옵션
				<td>
				<label for="searchOption1"></label>
				<select name="searchOption1" class="form-control" id="searchOption1">
							<option value="userid">아이디</option>
							<option value="name">이름</option>
							<option value="addr1">주소</option>
							<option value="all">아이디/이름/주소</option>
						</select>
			<div class="input-group">
				<input type="text" class="form-control" name="searchKeyword"
					value="${searchOp.searchKeyword}">
			</div>
				</td>
				</tr>
				<tr>
				<th>가입일</th>
				<td>
				<input type="date" id="startDate" name="startDate" value="${searchOp.startDate}">&nbsp;&nbsp;~&nbsp;&nbsp; 
				<input type="date" id="endDate" name="endDate" value="${searchOp.endDate}">
				</td>
				</tr>
				<tr>
				<th>분류</th>
				<td>
				<label for="searchOption2"></label>
				<select name="searchOption2" class="form-control" id="searchOption2">
							<option value="user">이용</option>
							<option value="quit">탈퇴</option>
							<option value="all">전체</option>
						</select>
				</td>
				</tr>
				<tr>
				<th>정렬기준</th>
				<td>
				<select name="orderOption" class="form-control" id="orderOption">
								<option value="userid">아이디순</option>
								<option value="name">이름순</option>
								<option value="join_date">최근가입순</option>
							</select>
				</td>
				</tr>
			</table>
			<div class="input-group-btn">
					<button class="btn btn-default" type="button" id="btnSearch">
						검색  <i class="glyphicon glyphicon-search"></i>
					</button>
				</div>
			</div>

			<table class="table table-hover">
				<thead>
					<tr>
						<th>#</th>
						<th>No</th>
						<th>아이디</th>
						<th>이름</th>
						<th>이메일</th>
						<th>성별</th>
						<th>전화번호</th>
						<th>주소</th>
						<th>가입일</th>
					</tr>
				</thead>
				<c:forEach var="dto" varStatus="loop" items="${list}">
					<tbody>
						<tr>
							<td><input name="userids" type="checkbox"
								value="${dto.userid}"></td>
							<td>${dto.rn}</td>
							<td><a href="#">${dto.userid}</a></td>
							<td>${dto.name}</td>
							<td>${dto.email}</td>
							<td>${dto.gender}</td>
							<td>${dto.hp}</td>
							<td>${dto.addr1}</td>
							<td><fmt:formatDate value="${dto.join_date}" type="date"
									pattern="yyyy-MM-dd" /></td>
						</tr>
					</tbody>
				</c:forEach>
			</table>
			<div class="row" align="center">
				<div class="col-sm-12">
					<ul class="pagination pagination">
						<c:if test="${pager.curPage > 1}">
							<li><a href="#" onclick="adminList('1')">첫 페이지</a></li>
						</c:if>
						<c:if test="${pager.curBlock > 1}">
							<li><a href="#" onclick="adminList('${pager.prevPage}')">이전</a></li>
						</c:if>
						<c:forEach var="page" begin="${pager.blockStartPage}"
							end="${pager.blockEndPage}">
							<c:choose>
								<c:when test="${page == pager.curPage}">
									<li><a href="#" style="text-decoration: underline;">${page}</a></li>
								</c:when>
								<c:otherwise>
									<li><a href="#" onclick="adminList('${page}')">${page}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${pager.curBlock < pager.totBlock}">
							<li><a href="#" onclick="adminList('${pager.nextPage}')">다음</a></li>
						</c:if>
						<c:if test="${pager.curPage < pager.totPage}">
							<li><a href="#" onclick="adminList('${pager.totPage}')">마지막
									페이지</a></li>
						</c:if>
					</ul>
				</div>
			</div>
			<button id="btnDelete" type="button" class="btn btn-default">탈퇴처리</button>
		</form>
	</div>
</body>
</html>