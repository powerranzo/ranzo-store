...
<body>
	...
	<%@ include file="../include/adminDashboard.jspf"%>
		...
		<table class="adminTable2">
			...
			<th>노출기간</th>
			<td colspan="2">
			<input type="date" id="startDate" name="startDate" value="${ pop.searchOp.startDate}">
			<span style="float:left; padding:7px;">~</span>
			<input type="date" id="endDate" name="endDate" value="${ pop.searchOp.endDate}">
			</td>
			</tr>
			<tr>
			<th>정렬기준</th>
		...
		<c:forEach var="pop" varStatus="loop" items="${pop.popup_list}">
			...
			<td>
				...
				<button class="btn btn-sm" type="button" id="" onclick="popupView('${pop.no}')">
				<span class="glyphicon glyphicon-check"></span>&nbsp;수정
				</button>
				<button class="btn btn-sm" type="button" id="" onclick="popupDel('${pop.no}')">
				<span class="glyphicon glyphicon-minus-sign"></span>&nbsp;종료
				</button>
				</c:when>
				<c:otherwise>
				<button class="btn btn-sm" type="button" id="" onclick="popupShow('${pop.no}')">
		...
		<c:if test="${pop.pager.curBlock > 1}">
			...
			<c:if test="${pop.pager.curBlock > 1}">
			<a href="#" onclick="popupList('${pop.pager.prevPage}')"> 
			<i class="fa-solid fa-less-than"></i></a>
		</c:if>
		<c:forEach var="page" begin="${pop.pager.blockStartPage}" end="${pop.pager.blockEndPage}">
			<c:choose>
